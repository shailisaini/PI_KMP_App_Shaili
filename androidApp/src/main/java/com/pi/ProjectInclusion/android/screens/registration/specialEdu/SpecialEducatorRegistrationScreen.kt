...
@Composable
fun SpecialEducatorRegistrationScreen(
    onNext: () -> Unit,
    onBack: () -> Unit,
    ...
    Surface(
        ...
        Column(
            ...
            SpeEducatorScreenUI(context, onBack = onBack, onNext = onNext, viewModel = viewModel)
        }
    }
}
...
fun SpeEducatorScreenUI(
    ...
    var isBottomReasonVisible by rememberSaveable { mutableStateOf(false) }
    ...
    var allReasons = remember { mutableStateListOf<ReasonListResponse.ReasonResponse>() }
    var allUdiseDetails =
    ...
    var reasonSelectedId = remember { mutableIntStateOf(-1) }
    ...
    var msgReason = stringResource(R.string.key_select_reason)
    ...
    LaunchedEffect(allReasonState) {
        when {
            allReasonState.isLoading -> {
                isDialogVisible = true
            }
            ...
            allReasonState.success != null -> {
                ...
                if (allReasonState.success?.status == true) {
                    ...
                    println("All Reason list data :- $allReasons")
                }
                isDialogVisible = false
            }
        }
